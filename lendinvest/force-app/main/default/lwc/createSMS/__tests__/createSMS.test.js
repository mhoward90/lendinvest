...
describe('c-create-sms', () => {
  ...
  it('should display the input components', async () => {
    ...
    const phoneNumber = element.shadowRoot.querySelector('lightning-formatted-phone');
  ...
  it('should update the number of characters remaining', async () => {
    ...
    textArea.value = 'I have written some text to reduce the number of characters remaining';
    textArea.dispatchEvent(new CustomEvent('change'));

    await flushPromises();
    const remainingText = element.shadowRoot.querySelector('[data-id="remaining_characters"]');
    expect(remainingText.textContent).toBe('Number of characters remaining: 91');
  });
});